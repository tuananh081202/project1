...
const PaymentCartList = () => {
    ...
    //const[PaymentCartList,setPaymentCartList]=useState([])
    ...
    const columns = [
        ...
        },
        {
            ...
            element: row  =>(
                <>
                    <Link to={`/paymentcart/${row.id}`} className='btn btn-sm btn-info me-1'><i class="fa-solid fa-book"></i> Read </Link>
                    <Link to={`/paymentcart/edit/${row.id}`} className='btn btn-sm btn-warning me-1' ><i className="fa fa-pencil"></i> Edit </Link>
    ...
    const requestDeleteApi = () =>{
       ...
       dispatch(actions.controlLoading(true))
        requestApi(`/paymentcart/${deleteItem}`,'DELETE',[]).then(response =>{
            setShowModal(false)
            setRefresh(Date.now())
        ...
        dispatch(actions.controlLoading(true))
        requestApi(`/paymentcart/multiple?ids=${selectedRows.toString()}`, 'DELETE', []).then(response => {
            setShowModal(false)
            setRefresh(Date.now())
            setSelectedRows([])
            dispatch(actions.controlLoading(false))
            ...
            complete:function(result) {
                ...
                if(rawCSV.length > 0){
                    if(rawCSV[0] && rawCSV.length ===8){
                        if(rawCSV[0][0] !== "id"
                        || rawCSV[0][1] !== "customerName"
                        || rawCSV[0][2] !== "paymentMethod"
                        || rawCSV[0][3] !== "totalPrice"
                        || rawCSV[0][4] !== "status"
                        || rawCSV[0][5] !== "created_at"
                        || rawCSV[0][6] !== "updated_at"
                        || rawCSV[0][7] !== "product"
                    ){
                        ...
                        rawCSV.map((item,index) =>{
                            if(index>0 && item.length === 8){
                                let obj = {}
                                obj[0] = item.id
                                obj[1] = item.customerName
                                obj[2] = item.paymentMethod
                                obj[3] = item.totalPrice
                                obj[4] = item.status
                                obj[5] = item.created_at
                                obj[6] = item.updated_at
                                obj[7] = item.product
                                result.push(obj)
    ...
    const getPaymentCartExport = (_event,done) =>{
        let result = []
        if (PaymentCartList && PaymentCartList.length >0){
            result.push(['id','customerName','paymentMethod','product','totalPrice','status','created_at','updated_at']);
            PaymentCartList.map(item =>{
                let arr = [];
                
                arr[0] = item.id
                arr[1] = item.customerName
                arr[2] = item.paymentMethod
                arr[3] = item.totalPrice
                arr[4] = item.status
            ...
            <main>
                <div className="container-fluid px-4">
                    ...
                    <div className='mb-3'>
                        ...
                        <Link className='btn btn-sm btn-success me-1' to='/paymentcart/add'><i className='fa fa-plus'></i>Add new</Link>
                        ...
                        target="_blank"
                        asyncOnClick={true}
                        onClick={(event,done) =>getPaymentCartExport(event,done)}
                        ><i className='fa-solid fa-file-arrow-down'></i> Export Excel </CSVLink>
                        
                    </div>
                    <Table 
                    name="List PaymentCart"
                    data={paymentcart}
                    columns={columns}
                    numOfPage={numOfPage}
